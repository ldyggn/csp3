...
export default function ArchiveProduct({product, isActive, fetchData}) {
	...
	const archiveToggle = (productId) => {
		fetch(`http://ec2-18-217-154-136.us-east-2.compute.amazonaws.com/b5/products/${productId}/archive`, {
		method: 'PATCH',
			headers: {
	...
	const activateToggle = (productId) => {
		fetch(`http://ec2-18-217-154-136.us-east-2.compute.amazonaws.com/b5/products/${productId}/activate`, {
			method: 'PATCH',
			headers: {
				'Content-Type': 'application/json',
				Authorization: `Bearer ${localStorage.getItem('token')}`
